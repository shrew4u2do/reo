...
class BaichuanFramer:
    ...
    as (cmd_id, mclass, payload_off, body_bytes, raw_message_bytes). raw is the
    full on-wire message (header+body) so callers can selectively forward
    non-media messages back to reolink_aio's parser."""

    def __init__(self) -> None:
        self.buf = bytearray()

    def feed(self, data: bytes) -> list[tuple[int, str, int, bytes, bytes]]:
        self.buf += data
        out = []
        while True:
            if len(self.buf) < 4:
            ...
            out.append((cmd_id, mclass, poff, body, raw))
            del self.buf[:total]
        return out
